...
export function parseManualCandidatesForFaction(
  ...
  // ---- HARD GUARD: prevent undefined faction propagation ----
  if (!faction) {
    traceFaction(
      'src/compute/border-labeler/functions/parse-manual-candidates-for-faction.ts',
      'manual-candidate-undefined-faction',
      faction as unknown as Faction
    );
    return [];
  }
  ...
  return configs.map((config, configIndex) => {
    ...
    const labelDetails =
      factionNameTokens[config.labelVariant || BorderLabelVariant.SingleLine];

    const pointByDegree =
      pointOnUnitCircleByPercentValue((config.angle || 0) / 360);
    ...
    const baseline: Edge2d = {
      p1: {
        x: config.anchor.x - baselineHalfVector.a,
        y: config.anchor.y - baselineHalfVector.b
      },
      p2: {
        x: config.anchor.x + baselineHalfVector.a,
        y: config.anchor.y + baselineHalfVector.b
    ...
    const labelBaselines: BorderLabelBaselines = {
      ...
      top: {
        p1: {
          x: baseline.p1.x + perpVector.a,
          y: baseline.p1.y + perpVector.b
        },
        p2: {
          x: baseline.p2.x + perpVector.a,
          y: baseline.p2.y + perpVector.b
        },
      },
    };
    ...
    if (config.labelVariant === BorderLabelVariant.MultiLine) {
      scaleVector(
        perpVector,
        factionNameTokens[BorderLabelVariant.SingleLine]?.height || 1
      );
