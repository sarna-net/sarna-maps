...
export class VoronoiBorder {
  ...
  private static generateBorderEdges(voronoiNodes: VoronoiNode[], vertices: DelaunayVertex[]) {
    ...
    for(let nodeIdx = 0, triIdx = 0; nodeIdx < voronoiNodes.length; nodeIdx++, triIdx += 3) {
      ...
      // A-B and A-C edges:
      vertex1.adjacentTriIndices.forEach((vert1AdjTriIndex) => {
        // trivially, the current triangle is not its own neighbor
        if(vert1AdjTriIndex === triIdx) { return }
        vertex2.adjacentTriIndices.forEach((vert2AdjTriIndex) => {
          if(vert1AdjTriIndex === vert2AdjTriIndex) {
            voronoiNode.neighborNodeIndices.push(vert1AdjTriIndex / 3);
          }
        });
        vertex3.adjacentTriIndices.forEach((vert3AdjTriIndex) => {
      ...
      // B-C edges
      vertex2.adjacentTriIndices.forEach((vert2AdjTriIndex) => {
        if(vert2AdjTriIndex === triIdx) { return }
        vertex3.adjacentTriIndices.forEach((vert3AdjTriIndex) => {
      ...
      voronoiNode.neighborNodeIndices.forEach((neighborIdx) => {
        ...
        if(sharedVertexIndices.length >= 2) {
          const color1 = vertices[sharedVertexIndices[0]].color;
          const color2 = vertices[sharedVertexIndices[1]].color;
          if(color1 !== color2) {
            const borderEdge = {
      ...
      while(unprocessedEdges.length > 0) {
        if(!currentEdge) {
          ...
          for(let unprocessedEdgeIdx = 0; unprocessedEdgeIdx < unprocessedEdges.length; unprocessedEdgeIdx++) {
            const potentialNeighborEdge = unprocessedEdges[unprocessedEdgeIdx];
        ...
        if(!currentEdge) {
          throw new Error(
            `Error while generating border loops for ${color}: ` +
        ...
        if (currentLoop.minEdgeIdx < 0 || currentEdge.node2.x  < currentLoop.edges[currentLoop.minEdgeIdx].node2.x) {
  ...
  private static pullEdgeLoop(loop: BorderEdgeLoop, vertices: DelaunayVertex[], borderSeparation: number) {
    ...
    for(let curEdgeIdx = 0; curEdgeIdx < loop.edges.length; curEdgeIdx++) {
      ...
      // const rightSideVertexIdx = currentEdge.color1 === currentEdge.rightColor ? currentEdge.vertex1Idx : currentEdge.vertex2Idx;
  ...
  private static generateEdgeControlPoints(loop: BorderEdgeLoop, tension: number) {
    for (let edgeIndex = 0; edgeIndex < loop.edges.length; edgeIndex++) {
      const currentEdge = loop.edges[edgeIndex];
      const nextEdge = loop.edges[(edgeIndex + 1) % loop.edges.length];

      const point1 = currentEdge.node1;
      const point2 = nextEdge.node1;
      const point3 = nextEdge.node2;
      ...
      const dist12 = distance(point1, point2);
      const dist23 = distance(point2, point3);
      ...
      const w = point3.x - point1.x;
      const h = point3.y - point1.y;
